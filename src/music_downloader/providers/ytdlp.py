...
class YtDlpProvider(Provider):
    ...
    def inspect(self, url: str) -> SourceTrack:
        ...
        return SourceTrack(self.name,sid,str(info.get("title") or Path(urlparse(url).path).stem),url,
                           info.get("artist") or info.get("uploader"),info.get("album"),
                           int(float(duration)*1000) if duration is not None else None,
                           info.get("thumbnail"))
    def playlist(self, url: str):
        ...
        for position, entry in enumerate(info.get("entries") or []):
            ...
            source = SourceTrack(self.name,str(entry["id"]),str(entry.get("title") or entry["id"]),
                                 entry.get("url") or entry.get("webpage_url"),
                                 entry.get("artist") or entry.get("uploader"),None,
                                 int(float(duration)*1000) if duration is not None else None,
                                 entry.get("thumbnail"))
    ...
    def download(self, source: SourceTrack, destination: str) -> None:
        ...
        self._run(["--no-playlist","--format","bestaudio/best","--extract-audio","--audio-format","mp3",
                   "--output",str(target),source.url or source.source_id])
